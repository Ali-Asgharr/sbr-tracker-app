...
def run_phase2(tracker_bytes, submission_bytes, log):
    ...
    # ── Apply Lag Time + 30/45-Day + No Response Letter Status formulas ───────
    lag = elec = paper = 0
    for r in range(2, last_data_row + 1):
        bst = ws.cell(row=r, column=21).value
        if normalize(bst) != '':
            ws.cell(row=r, column=23).value = f'=TODAY()-V{r}'
            lag += 1
    ...
    buf = io.BytesIO(); wb.save(buf); buf.seek(0)
    return buf.read(), {"matched": matched, "not_found": not_found, "uv": uv,
                        "skipped_na": skipped_na, "skipped_existing": skipped_existing,
                        "lag": lag, "elec": elec, "paper": paper}
...
# ─────────────────────────────────────────────────────────────────────────────
# ADMIN PANEL — SECRET RESTART BUTTON
# ─────────────────────────────────────────────────────────────────────────────
...
with st.expander("⚙️ Admin Panel (Private)", expanded=False):
    pwd = st.text_input("Enter Admin Password", type="password", key="admin_pwd")

    if pwd == ADMIN_PASSWORD:
        st.success("✅ Access granted")

        confirm = st.checkbox("Confirm restart")

        if st.button("🔄 Force Restart App") and confirm:
            try:
                with open("trigger_restart.txt", "w") as f:
                    f.write(str(datetime.now()))

                st.warning("🔄 Restart triggered — app will reload in a few seconds...")
            except Exception as e:
