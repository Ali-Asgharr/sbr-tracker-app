...
[data-testid="stFileUploader"] section div {color:#111 !important;}
[data-testid="stFileUploader"] label {color:#111 !important;font-weight:600;}
.filebox {background:#f1f3f5;border:1px solid #d0d7de;padding:8px 12px;border-radius:6px;margin-top:6px;font-size:12px;color:#111}
...
with col1:
    tracker_file = st.file_uploader("Upload MASTER TRACKER file (.xlsx)", type=["xlsx"], key="f_t")
with col2:
    bdr_file = st.file_uploader("Upload BDR file (.csv)", type=["csv"], key="f_b")

col3, col4 = st.columns(2)
with col3:
    submission_file = st.file_uploader("Upload SUBMISSION REPORT file (.xlsx / .csv)", type=["xlsx","csv"], key="f_s")
with col4:
    payment_file = st.file_uploader("Upload PAYMENT REPORT file (.csv / .xlsx)", type=["csv","xlsx"], key="f_p")

doclog_file = st.file_uploader("Upload DOC LOG file (.csv / .xlsx)", type=["csv","xlsx"], key="f_d")
