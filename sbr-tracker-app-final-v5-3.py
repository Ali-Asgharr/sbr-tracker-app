...
with col1:
    tracker_file = st.file_uploader("Master-Tracker (.xlsx)", type=["xlsx"], key=f"f_t_{_v}")
with col2:
    bdr_file = st.file_uploader("BDR (.csv)", type=["csv"], key=f"f_b_{_v}")

col3, col4 = st.columns(2)
with col3:
    submission_file = st.file_uploader("Submission Report (.xlsx or .csv)", type=["xlsx","csv"], key=f"f_s_{_v}")
with col4:
    payment_file = st.file_uploader("Payment Report (.csv or .xlsx)", type=["csv","xlsx"], key=f"f_p_{_v}")

doclog_file = st.file_uploader("Doc-Log (.csv or .xlsx)", type=["csv","xlsx"], key=f"f_d_{_v}")
