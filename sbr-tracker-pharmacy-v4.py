...
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="SBR Tracker Automation — Pharmacy",
    page_icon="📊",
    layout="wide",
...
_keys = ["p1_out","p2_out","p3_out","p4_out",
         ...
         "run_log","error_phase","_running","_upload_v"]
for k in _keys:
    if k not in st.session_state:
        st.session_state[k] = 0 if k == "_upload_v" else ([] if k == "run_log" else None)
...
def run_phase2(tracker_bytes, submission_bytes, log):
    ...
    # Only "2025-2026" worksheet is expected — "2021 to 2024" has been removed
    # by the director as it is old data no longer needed for processing.
    try:
        submission = pd.read_csv(io.BytesIO(submission_bytes), low_memory=False)
        ...
        if target in xl.sheet_names:
            submission = xl.parse(target)
            log_line(log, f"✓ Submission loaded · sheet '{target}' · {len(submission):,} rows", "ok")
        else:
            # Fallback: use whatever the first sheet is (future-proofs rename)
            submission = xl.parse(xl.sheet_names[0])
            log_line(log, f"✓ Submission loaded · sheet '{xl.sheet_names[0]}' · {len(submission):,} rows", "ok")

    # ── Build in-memory lookup: Study ID → {EDI Service Type, Submission Date} ─
    # keep='last' so the most recent submission record wins for duplicate IDs
    ...
    # ── Single-pass merge: resolve lookup in Python, write directly to cols 21/22 ─
    # No temp columns inserted/deleted in the sheet — all staging happens in memory.
    wb = load_workbook(io.BytesIO(tracker_bytes)); ws = wb["Timeline"]

    matched = not_found = uv = skipped_na = skipped_existing = 0
    for r in range(2, last_data_row + 1):
        ...
        if k not in lkp:
            # Study ID absent from 2025-2026 sheet — leave cols 21/22 untouched
            not_found += 1
            continue

        # Resolve EDI Service Type and Submission Date entirely in Python
        sub = lkp[k]
        raw = sub.get('EDI Service Type')
        dv  = safe_date(sub.get('Submission Date'))

        clean = None
        if raw is not None:
            try:
                if not math.isnan(float(raw)):
                    clean = str(raw).strip() or None
            except (ValueError, TypeError):
                clean = str(raw).strip() or None

        # Nothing useful came back from lookup
        if clean is None and dv is None:
            not_found += 1
            continue

        matched += 1

        # Guard 1: skip rows where the looked-up date is an NA-like sentinel
        dv_str = str(dv).strip() if dv is not None else ''
        if dv_str in ('nan', 'NaT', 'None', 'NA', 'N/A', ''):
            skipped_na += 1
            continue

        # Guard 2: never overwrite col 21 when it already has a value
        existing_bst = ws.cell(row=r, column=21).value
        existing_bst_str = str(existing_bst).strip() if existing_bst is not None else ''
        if existing_bst_str != '':
            skipped_existing += 1
            continue

        # Safe to write — col 21 is blank and date is valid
        ws.cell(row=r, column=21).value = clean
        ws.cell(row=r, column=22).value = dv
        uv += 1

    log_line(log, f"✓ Submission data pulled · {matched:,} rows matched", "ok")
    log_line(log, f"✓ Blank rows updated: {uv:,} · Skipped (NA date): {skipped_na:,} · Skipped (already filled): {skipped_existing:,}", "ok")

    lag=elec=paper=0
    for r in range(2, last_data_row+1):
        bst=ws.cell(row=r,column=21).value
        if normalize(bst)!='': ws.cell(row=r,column=23).value=f'=TODAY()-V{r}'; lag+=1
        if is_electronic_or_blank(bst):
            ws.cell(row=r,column=24).value=f'=IF(W{r}>=30,"30 days passed","Under 30 Days")'
            ws.cell(row=r,column=27).value=(
                f'=IF(X{r}="Under 30 Days","No Action Required",'
                f'IF(T{r}="No","No Action Required",'
                f'IF(Y{r}="No Response","Send No Response Letter","Response Received")))'
            )
            elec+=1
        elif is_paper_type(bst):
            ws.cell(row=r,column=24).value=f'=IF(W{r}>=45,"45 days passed","Under 45 Days")'
            ws.cell(row=r,column=27).value=(
                f'=IF(X{r}="Under 45 Days","No Action Required",'
                f'IF(T{r}="No","No Action Required",'
                f'IF(Y{r}="No Response","Send No Response Letter","Response Received")))'
            )
            paper+=1
    ...
    log_line(log, f"✓ 30/45 Days + No Response Letter Status · Electronic/blank: {elec:,} rows · Paper: {paper:,} rows", "ok")
    log_line(log, "✓ PHASE 2 COMPLETE → output ready for download", "ok")

    wb.calculation.calcMode = 'auto'
    buf = io.BytesIO(); wb.save(buf); buf.seek(0)
    return buf.read(), {"matched":matched,"not_found":not_found,"uv":uv,"skipped_na":skipped_na,"skipped_existing":skipped_existing,"lag":lag,"elec":elec,"paper":paper}
...
def run_phase3(tracker_bytes, payment_bytes, log):
    ...
    for r in range(2, ldr+1):
        ...
        if str(pr).strip().upper()!="NO":
            ay+=1
            continue  # already-YES rows handled in the second pass below
    ...
    # Second pass: overwrite Last Payment Date for already-YES rows with the latest
    # payment date from the payment report. This replicates the manual step of clearing
    # filters, unselecting NA, and overwriting dates on old YES studies — needed because
    # payments arrive in chunks and the tracker must always show the most recent date.
    for r in range(2, ldr+1):
        sr = ws.cell(row=r,column=1).value
        if sr is None: continue
        sid = str(sr).strip()
        if sid not in plkp: continue
        pr = ws.cell(row=r,column=28).value
        if str(pr).strip().upper() != "YES": continue  # only already-YES rows
        npd = plkp[sid]
        ed = safe_to_datetime(ws.cell(row=r,column=29).value)
        if ed is not None:
            if npd.date() > ed.date():
                ws.cell(row=r,column=29).value = to_python_datetime(npd); ayu+=1
            else:
                ws.cell(row=r,column=29).value = to_python_datetime(ed); ayk+=1
        else:
            ws.cell(row=r,column=29).value = to_python_datetime(npd); ayn+=1
    ...
    log_line(log, f"✓ Matched: {matched:,} · NO→YES: {updated:,} · Already YES (date refreshed): {ay:,}", "ok")
    log_line(log, f"✓ Already YES · Updated to newer date: {ayu:,} · Kept existing date: {ayk:,} · No prior date: {ayn:,}", "ok")
...
def build_doclog_lookup(doclog_bytes, log):
    ...
    cols=list(df.columns); csid=cols[6]; csub=cols[8]; cdt=cols[18]  # Pharmacy: date column is 19th (index 18), not 20th
    ...
    log_line(log, f"✓ Doc-Log lookup · {len(afl):,} unique Study IDs (pharmacy col index 18)", "ok")
    return afl, adt, adl
...
def run_phase4(tracker_bytes, doclog_bytes, log):
    ...
    s26=s27=s28=ndl=s29=s29n=s29u=s30u=s30d=s30n=s30p=0
    s31v=s31f=s31p=s31n=s32=0
    flagged=[]; sc=first_of_current_month()

    # ── Step 29 date-window: current month + next 2 months ──────────────────────
    today = date.today()
    ...
    # Build accepted month range: current month, +1, +2
    def _add_months(d, n):
        m = d.month - 1 + n
        y = d.year + m // 12
        m = m % 12 + 1
    ...
    window_start = _month_start(today.year, today.month)
    window_end   = _month_end(_add_months(today, 2).year, _add_months(today, 2).month)

    MIN_VALID_AK = date(2000, 1, 1)   # anything older than this is a junk/1900 date

    # Prepare Manual Review sheet (created fresh each run)
    MR_SHEET = "Manual Review - Billing Cycle"
    if MR_SHEET in wb.sheetnames:
        del wb[MR_SHEET]
    mr_ws = wb.create_sheet(MR_SHEET)
    # Write header row
    for ci, h in enumerate(headers, 1):
        c = mr_ws.cell(row=1, column=ci, value=h)
    ...
    for r in range(2, ldr+1):
        ...
        if amn=='under billing cycle':
            # Recalculate AK date in Python (col 35 holds a formula that openpyxl
            # cannot evaluate — it was written this run so cache is None).
            # Formula logic: if Payment Received (AB) or EOR Received (AD) = Yes,
            # take MAX(Last Payment Date AC, Last EOR Date AE) + 60 days, else no date.
            ab_val = ws.cell(row=r, column=CAB).value
            ad_val = ws.cell(row=r, column=CAD).value
            response_received = (normalize(ab_val) == 'yes' or normalize(ad_val) == 'yes')

            if response_received:
                ac_date = to_date(ws.cell(row=r, column=CAC).value)
                ae_date = to_date(ws.cell(row=r, column=CAE).value)
                valid_dates = [d for d in [ac_date, ae_date] if d is not None]
                ak_date = max(valid_dates) + timedelta(days=60) if valid_dates else None
            else:
                ak_date = None

            # Treat pre-2000 dates as junk
            if ak_date is not None and ak_date < MIN_VALID_AK:
                ak_date = None

            if ak_date is None:
                s29n += 1
            elif window_start <= ak_date <= window_end:
                ws.cell(row=r, column=CAM).value = "PENDING SBR"
                s29 += 1
            else:
                # Outside accepted window → copy to Manual Review sheet
                row_data = [ws.cell(row=r, column=c).value for c in range(1, ws.max_column + 1)]
                for ci, val in enumerate(row_data, 1):
                    mc = mr_ws.cell(row=mr_row, column=ci, value=val)
                    mc.fill = PatternFill("solid", fgColor="FFF3CD")
                    if isinstance(val, (datetime, date)):
                        mc.number_format = 'M/D/YYYY'
                        mc.alignment = Alignment(horizontal='center')
                mr_row += 1
                s29_manual += 1
        else:
            s29u += 1
        # ── Step 30 (updated logic) ───────────────────────────────────────────
        # 1. Skip protected statuses
        # 2. Compare AE vs AG: only update AE when AG is strictly newer than AE
        # 3. After update, recompute SOL Date (AK = MAX(AE,AC)+60) in Python
        # 4. Only mark PENDING SBR if AK falls in current month + next 2 months
        #    Otherwise leave the status unchanged — do NOT blanket-set all to pending
        am=ws.cell(row=r,column=CAM).value; amn=normalize(am)
        ...
        # Only update AE when AG is strictly greater (newer) than AE
        if ae30 is not None and ag30 <= ae30:
            s30d += 1
            # AE is already up-to-date — still check SOL window below using existing AE
            ae_for_sol = ae30
        else:
            # AG is newer (or AE is blank) — update AE from AG
            write_date_cell(ws.cell(row=r, column=CAE), ag30)
            ae_for_sol = ag30
            s30u += 1

        # Recompute SOL Date (AK) in Python: MAX(AE, AC) + 60 days
        ab30 = ws.cell(row=r, column=CAB).value
        ad30 = ws.cell(row=r, column=CAD).value
        response_received_30 = (normalize(ab30) == 'yes' or normalize(ad30) == 'yes')
        if response_received_30:
            ac30 = to_date(ws.cell(row=r, column=CAC).value)
            valid30 = [d for d in [ac30, ae_for_sol] if d is not None]
            ak30 = max(valid30) + timedelta(days=60) if valid30 else None
        else:
            ak30 = None
        if ak30 is not None and ak30 < MIN_VALID_AK:
            ak30 = None
    ...
    log_line(log, f"✓ Step 30: AE updated={s30u} · AE already current={s30d} · Marked PENDING SBR (in SOL window)=counted above · Protected={s30p}", "ok")
    ...
    # ── Step 32: PENDING SBR rows → Timely Response = Yes ────────────────────
    log_line(log, "→ Step 32: Setting Timely Response=Yes for PENDING SBR rows...", "info")
    s32 = 0
    for r in range(2, ldr+1):
        sbr_val = ws.cell(row=r, column=CAM).value
        if sbr_val is not None and 'pending sbr' in str(sbr_val).strip().lower():
            ws.cell(row=r, column=CTM).value = "Yes"
            ws.cell(row=r, column=CTM).alignment = Alignment(horizontal='center')
            s32 += 1
    log_line(log, f"✓ Step 32: Timely Response → Yes for {s32} PENDING SBR rows", "ok")
    log_line(log, "✓ PHASE 4 COMPLETE → All phases done! ✓", "ok")

    buf=io.BytesIO(); wb.save(buf); buf.seek(0)
    return buf.read(), {"z":s26+s27,"ad":s28,"pending":s29,"manual_review":s29_manual,"step30":s30u,"anomalies":s31f,"timely":s32}
...
<div class="sbr-topbar">
  <div class="sbr-brand">
    ...
    <div>
      <div class="sbr-brand-name">SBR Tracker Automation — Pharmacy</div>
      <div class="sbr-brand-sub">Pharmacy Providers · Steps 1–34 · Phases 1–4 · v2.0</div>
    </div>
  </div>
...
_v = st.session_state._upload_v
col1, col2 = st.columns(2)
with col1:
    tracker_file = st.file_uploader("Master-Tracker(.xlxs)", type=["xlsx"], key=f"f_t_{_v}")
with col2:
    bdr_file = st.file_uploader("BDR(.csv)", type=["csv"], key=f"f_b_{_v}")

col3, col4 = st.columns(2)
with col3:
    submission_file = st.file_uploader("Submission(.xlxs)", type=["xlsx","csv"], key=f"f_s_{_v}")
with col4:
    payment_file = st.file_uploader("Payment-Report(.xlxs)", type=["csv","xlsx"], key=f"f_p_{_v}")

doclog_file = st.file_uploader("Doc-Log(.csv)", type=["csv","xlsx"], key=f"f_d_{_v}")
...
r1 = uz_card(tracker_file,    "Master-Tracker(.xlxs)",  "xl") + uz_card(bdr_file,      "BDR(.csv)",         "csv")
r2 = uz_card(submission_file, "Submission(.xlxs)",    "xl") + uz_card(payment_file,  "Payment-Report(.xlxs)",  "csv")
r3 = uz_card(doclog_file,     "Doc-Log(.csv)",              "csv", wide=True)
...
if st.button("↺  Reset Everything", key="reset", use_container_width=True):
    ...
    # Increment version → forces Streamlit to render brand-new uploader widgets (clears files)
    st.session_state._upload_v = (st.session_state._upload_v or 0) + 1
...
<div style="text-align:center;padding:16px 0 8px;">
  <span style="font-size:10px;color:#bbb;font-family:'JetBrains Mono',monospace;">
    SBR Tracker Automation — Pharmacy · Steps 1–34 · Phases 1–4 · v2.0
  </span>
</div>""", unsafe_allow_html=True)
