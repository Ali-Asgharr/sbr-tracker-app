...
def run_phase2(tracker_bytes, submission_bytes, log):
    ...
    # New logic: no True/False formula column.
    # After lookup, skip any rows where the temp date (col 24) is NA.
    # Only update col 21 (Bill Submission Type) and col 22 (Bill Submission Date)
    # when col 21 is currently BLANK — existing values are never overwritten.
    uv = skipped_na = skipped_existing = 0
    for r in range(2, last_data_row+1):
        tw = ws.cell(row=r, column=23).value   # temp: EDI Service Type from lookup
        tx = ws.cell(row=r, column=24).value   # temp: Submission Date from lookup

        # Skip rows where lookup found nothing
        if tw is None and tx is None:
            continue

        # Skip rows where the looked-up date is an NA-like sentinel
        tx_str = str(tx).strip() if tx is not None else ''
        if tx_str in ('nan', 'NaT', 'None', 'NA', 'N/A', ''):
            skipped_na += 1
            continue

        # Only update when Bill Submission Type (col 21) is currently blank
        ...
        ws.cell(row=r, column=21).value = tw
        ws.cell(row=r, column=22).value = tx
        uv += 1

    ws.delete_cols(23, 3)
    log_line(log, f"✓ Temp columns removed · Blank rows updated: {uv:,} · Skipped (NA date): {skipped_na:,} · Skipped (already filled): {skipped_existing:,}", "ok")
    ...
    # Step 14: Apply exact No Response Letter Status formula per bill type.
    # Electronic/blank rows use the 30-day threshold check on col X;
    # paper rows use the 45-day threshold check on col X.
    # Both then fall through to the same T/Y logic.
    nrls_elec = nrls_paper = 0
    for r in range(2, last_data_row+1):
        bst = ws.cell(row=r, column=21).value
        if normalize(bst) == "" and ws.cell(row=r, column=24).value is None:
            continue  # row has no submission data at all — leave blank
        if is_paper_type(bst):
            # Paper: 45-day formula
            ...
            # Electronic or blank: 30-day formula
            ws.cell(row=r, column=27).value = (
    ...
    log_line(log, f"✓ No Response Letter Status · Electronic/blank: {nrls_elec:,} rows (30-day) · Paper: {nrls_paper:,} rows (45-day)", "ok")
