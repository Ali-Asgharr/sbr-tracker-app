...
DEPLOYMENT NOTE — create .streamlit/config.toml with:
    [server]
    maxUploadSize = 500
    maxMessageSize = 500

This is required for Streamlit Cloud and local to accept files >200 MB.
The st.config.set_option() call below is a fallback for environments where
config.toml may be missing, but config.toml takes precedence on Streamlit Cloud.
"""
...
# ── Robust loader for large CSV/Excel files (200MB+) ─────────────────────────
def load_large_csv(file_bytes, usecols=None, dtype=None):
    """Memory-efficient CSV loader using chunked reads.
    Falls back to loading all columns if usecols are not present."""
    def _read(cols):
        chunks = []
    ...
    try:
        return _read(usecols)
    except ValueError:
        # Requested usecols not found — load all columns
        return _read(None)


def load_large_excel_readonly(file_bytes, sheet_name=None):
    """Memory-efficient Excel loader using read_only mode (avoids full parse)."""
...
def run_phase1(tracker_bytes, bdr_bytes, log):
    ...
    # ── Chunked BDR load — handles 200MB+ CSV without freezing ─────────────────
    log_line(log, "→ Loading BDR (chunked, memory-safe)...", "info")
    bdr = load_large_csv(bdr_bytes)
    log_line(log, f"✓ BDR loaded · {len(bdr):,} rows", "ok")

    # ── Read-only tracker pass for ID comparison ────────────────────────────────
    log_line(log, "→ Loading Master-Tracker (read-only pass)...", "info")
    tracker = load_large_excel_readonly(tracker_bytes, sheet_name="Timeline")
    ...
    # ── Build slim lookup (only needed columns) before openpyxl write ───────────
    _lookup_cols = ['APPOINTMENTTYPE','AMOUNTBILLED','PAID','WRITEOFF',
                    'OUTSTANDING','OMFS','OWNER_NAME','PORTFOLIO_NAME',
                    'SUB OWNER_NAME','STATUS']
    bdr_lookup = (
        bdr.drop_duplicates(subset='STUDY_ID', keep='last')
           .set_index('STUDY_ID')[_lookup_cols]
           .to_dict('index')
    )
    del bdr; gc.collect()   # free BDR before opening large openpyxl workbook
...
def run_phase2(tracker_bytes, submission_bytes, log):
    ...
    # ── Read tracker row-count via fast read-only pass ──────────────────────────
    tracker_df    = load_large_excel_readonly(tracker_bytes, sheet_name="Timeline")
    total_rows    = tracker_df.shape[0]
    last_data_row = total_rows + 1
    del tracker_df; gc.collect()

    # ── Load submission: chunked CSV preferred; Excel fallback uses usecols ──────
    _sub_cols = ['STUDY_ID', 'EDI Service Type', 'Submission Date']
    try:
        submission = load_large_csv(submission_bytes, usecols=_sub_cols)
...
def run_phase3(tracker_bytes, payment_bytes, log):
    ...
    # ── Fast row-count via read-only pass ───────────────────────────────────────
    tracker_df = load_large_excel_readonly(tracker_bytes, sheet_name="Timeline")
    total_rows = tracker_df.shape[0]
    del tracker_df; gc.collect()

    # ── Load payment report: chunked CSV preferred ──────────────────────────────
    try:
        pay = load_large_csv(payment_bytes, usecols=['STUDY_ID', 'PAYMENTDATE'])
        log_line(log, f"✓ Payment Report loaded · CSV · {len(pay):,} rows", "ok")
    except Exception:
        try:
            pay = load_large_excel_readonly(payment_bytes)
            log_line(log, f"✓ Payment Report loaded · Excel read-only · {len(pay):,} rows", "ok")
        except Exception:
...
def build_doclog_lookup(doclog_bytes, log):
    try:
        df = load_large_csv(doclog_bytes)
        log_line(log, f"✓ Doc-Log loaded · CSV · {len(df):,} rows", "ok")
    except Exception:
        try:
            df = load_large_excel_readonly(doclog_bytes)
            log_line(log, f"✓ Doc-Log loaded · Excel read-only · {len(df):,} rows", "ok")
        except Exception:
...
def run_phase4(tracker_bytes, doclog_bytes, log):
    ...
    # ── Step 29 date-window: current month, last month, next 2 months ──────────
    today = date.today()
    ...
    # Build accepted month range: last month, current month, +1, +2
    def _add_months(d, n):
        m = d.month - 1 + n
        y = d.year + m // 12
        m = m % 12 + 1
    ...
    window_start = _month_start(_add_months(today, -1).year, _add_months(today, -1).month)
    ...
    buf=io.BytesIO(); wb.save(buf); buf.seek(0)
    return buf.read(), {"z":s26+s27,"ad":s28,"pending":s29,"manual_review":s29_manual,"step30":s30u,"anomalies":s31f,"timely":s32,"fix34":s34}
...
if run_clicked and all_ready:
    ...
    log_line(_init_log, "  Reading uploaded files into memory...", "info")
    # Read files one-at-a-time with a progress indicator so the UI stays responsive
    with st.spinner("Reading uploaded files…"):
        st.session_state._tb = tracker_file.read()
        log_line(_init_log, f"  ✓ Master-Tracker read ({fmt_bytes(len(st.session_state._tb))})", "info")
        st.session_state._bb = bdr_file.read()
        log_line(_init_log, f"  ✓ BDR read ({fmt_bytes(len(st.session_state._bb))})", "info")
        st.session_state._sb = submission_file.read()
        log_line(_init_log, f"  ✓ Submission Report read ({fmt_bytes(len(st.session_state._sb))})", "info")
        st.session_state._pb = payment_file.read()
        log_line(_init_log, f"  ✓ Payment Report read ({fmt_bytes(len(st.session_state._pb))})", "info")
        st.session_state._db = doclog_file.read()
        log_line(_init_log, f"  ✓ Doc-Log read ({fmt_bytes(len(st.session_state._db))})", "info")
    log_line(_init_log, "  All files loaded — starting Phase 1...", "ph")
...
if st.session_state._running:
    ...
    if next_phase == 1:
        ...
        try:
            o1,s1 = run_phase1(tb, bb, log)
            st.session_state.p1_out=o1; st.session_state.p1_stats=s1
            # Free BDR bytes — no longer needed after Phase 1
            if "_bb" in st.session_state: del st.session_state["_bb"]
        except Exception as e:
        ...
        try:
            ...
            st.session_state.p2_out=o2; st.session_state.p2_stats=s2
            # Free Submission + Phase1 output bytes — replaced by Phase2 output
            if "_sb" in st.session_state: del st.session_state["_sb"]
            st.session_state.p1_out = None
        except Exception as e:
        ...
        try:
            ...
            st.session_state.p3_out=o3; st.session_state.p3_stats=s3
            # Free Payment bytes + Phase2 output
            if "_pb" in st.session_state: del st.session_state["_pb"]
            st.session_state.p2_out = None
        except Exception as e:
        ...
        try:
            ...
            st.session_state.p4_out=o4; st.session_state.p4_stats=s4
            # Free DocLog bytes + Phase3 output
            if "_db" in st.session_state: del st.session_state["_db"]
            st.session_state.p3_out = None
        except Exception as e:
