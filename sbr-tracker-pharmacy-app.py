...
def run_phase4(tracker_bytes, doclog_bytes, log):
    ...
    for r in range(2, ldr+1):
        ...
        if amn=='under billing cycle':
            # Read AK column (col 35) — the date formula result
            ak_raw = ws.cell(row=r, column=35).value
            ak_date = to_date(ak_raw) if ak_raw is not None else None

            # Treat 1900-era or pre-2000 dates as no date
            if ak_date is not None and ak_date < MIN_VALID_AK:
                ak_date = None

            if ak_date is None:
                # No valid date — leave as-is, count as skipped
                s29n += 1
            elif window_start <= ak_date <= window_end:
                # Date falls within accepted window → PENDING SBR
                ws.cell(row=r, column=CAM).value = "PENDING SBR"
                s29 += 1
            else:
                # Date exists but outside window → copy to Manual Review sheet
