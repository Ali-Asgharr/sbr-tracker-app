...
.uz-name { font-size:12px; font-weight:600; color:#222; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.uz-card.ok .uz-name { color:#0C447C; background:#E6F1FB; padding:2px 6px; border-radius:4px; }
...
def run_phase2(tracker_bytes, submission_bytes, log):
    ...
    try:
        ...
        log_line(log, f"✓ Submission loaded · Excel · {len(submission):,} rows", "ok")
...
with col1:
    tracker_file = st.file_uploader("Upload Master-Tracker (.xlsx)", type=["xlsx"], key="f_t")
with col2:
    bdr_file = st.file_uploader("Upload BDR File (.csv)", type=["csv"], key="f_b")

col3, col4 = st.columns(2)
with col3:
    submission_file = st.file_uploader("Upload Submission Report (.xlsx / .csv)", type=["xlsx","csv"], key="f_s")
with col4:
    payment_file = st.file_uploader("Upload Payment Report (.xlsx / .csv)", type=["csv","xlsx"], key="f_p")

doclog_file = st.file_uploader("Upload Doc-Log (.xlsx / .csv)", type=["csv","xlsx"], key="f_d")
...
if run_log:
    if is_done:
        ...
        badge = (f'<div style="display:flex;align-items:center;gap:5px;">'
