...
# SESSION STATE
# ─────────────────────────────────────────────────────────────────────────────
_keys = ["p1_out","p3_out","p4_out",
         "p1_stats","p3_stats","p4_stats",
...
def build_doclog_lookup(doclog_bytes, log):
    ...
    cols=list(df.columns); csid=cols[6]
    # Use column names for DocumentSubType and Date Issued so both normal doc logs
    # (Date Issued at index 19) and pharmacy doc logs (Date Issued at index 18) work.
    if 'DocumentSubType' not in df.columns:
        raise ValueError("Doc-Log is missing required column 'DocumentSubType'")
    if 'Date Issued' not in df.columns:
        raise ValueError("Doc-Log is missing required column 'Date Issued'")
    csub = 'DocumentSubType'
    cdt  = 'Date Issued'
...
# PIPELINE RUNNER
# ─────────────────────────────────────────────────────────────────────────────
def run_all_phases(tb, bb, pb, db):
...
def pip_status():
    p1 = st.session_state.get("p1_out") is not None
    p3 = st.session_state.get("p3_out") is not None
    p4 = st.session_state.get("p4_out") is not None
    # 4 nodes: Upload, Phase 1, Phase 3, Phase 4
    s = [
        'done',                                       # Upload (always ready once we're here)
        'done' if p1 else 'active',                  # Phase 1
        'done' if p3 else ('active' if p1 else 'idle'),  # Phase 3
        'done' if p4 else ('active' if p3 else 'idle'),  # Phase 4
    ]
    return s
...
<div class="sbr-topbar">
  <div class="sbr-brand">
    ...
    <div>
      ...
      <div class="sbr-brand-sub">Steps 1–34 · Phases 1, 3–4 · v2.0</div>
    </div>
  </div>
...
# ─────────────────────────────────────────────────────────────────────────────
pip = pip_status()
labels = ["Upload", "Phase 1", "Phase 3", "Phase 4"]
...
for _uf, _lbl in [(tracker_file,"Master-Tracker"),(bdr_file,"BDR"),
                  (payment_file,"Payment Report"),(doclog_file,"Doc-Log")]:
...
r2 = uz_card(payment_file,   "Payment Report",       "csv", wide=False) + uz_card(doclog_file, "Doc-Log", "csv")
...
# RUN BUTTON
# ─────────────────────────────────────────────────────────────────────────────
all_files    = [tracker_file, bdr_file, payment_file, doclog_file]
file_names   = ["Master-Tracker.xlsx","BDR.csv","Payment Report","Doc-Log"]
...
if not all_ready:
    st.markdown(
        f'<div class="missing-hint">⚠ Missing: {", ".join(missing_list)} — upload all 4 files to enable</div>',
        unsafe_allow_html=True
    )

if run_clicked and all_ready:
    for k in ["p1_out","p3_out","p4_out","p1_stats","p3_stats","p4_stats","error_phase"]:
...
if st.session_state._running:
    ...
    # Determine which phase to run next (Phase 2 is skipped)
    p1_done = st.session_state.get("p1_out") is not None
    p3_done = st.session_state.get("p3_out") is not None
    p4_done = st.session_state.get("p4_out") is not None
    ...
    if not p1_done:
        ...
        render_log_inline(log, badge)
        try:
            o3,s3 = run_phase3(st.session_state.p1_out, pb, log)
            st.session_state.p3_out=o3; st.session_state.p3_stats=s3
        except Exception as e:
...
<div style="text-align:center;padding:16px 0 8px;">
  <span style="font-size:10px;color:#bbb;font-family:'JetBrains Mono',monospace;">
    SBR Tracker Automation · Steps 1–34 · Phases 1, 3–4 · v2.0
  </span>
</div>""", unsafe_allow_html=True)
