...
def run_phase2(tracker_bytes, submission_bytes, log):
    ...
    # Step 14 (new): Recalculate "No Response Letter Status" (col 27) for ALL rows.
    # Unified formula covers both electronic/blank (30-day) and paper (45-day) thresholds:
    # X already holds "Under 30 Days"/"30 days passed" or "Under 45 Days"/"45 days passed",
    # so checking either "Under" variant catches the not-yet-due condition for both types.
    nrls = 0
    for r in range(2, last_data_row+1):
        bst = ws.cell(row=r, column=21).value
        ...
        ws.cell(row=r, column=27).value = (
            f'=IF(OR(X{r}="Under 30 Days",X{r}="Under 45 Days"),"No Action Required",'
            f'IF(T{r}="No","No Action Required",'
            f'IF(Y{r}="No Response","Send No Response Letter","Response Received")))'
        )
        nrls += 1

    log_line(log, f"✓ No Response Letter Status recalculated · {nrls:,} rows", "ok")
